	...
	<div class="comment-list">
		...
		<div v-for="(item, index) in list" :key="index" class="item">
			<div class="item-header">
				...
				<div class="info">
					<div class="name">
						{{ item.user.name }}{{ item.user.type === 0 ? '(作者)' : '' }}
					</div>
			...
			<div class="item-comment">
				<div class="message" @click="showCommentModal(item._id, item.user)">
					<el-button size="small">回复</el-button>
				</div>
			</div>
			<div class="item-other" v-for="e in item.other.comments" :key="e._id">
				<div class="item-header">
					<div class="author">
						<div class="avatar">
							<img
					...
					<div class="info">
						<div class="name">
							{{ e.user.name }}{{ e.user.type === 0 ? '(作者)' : '' }}
						</div>
						<div class="time">{{ formatTime(e.create_time) }}</div>
					</div>
				</div>
				<div class="comment-detail">
					{{ '@' + e.to_user.name }}{{ e.to_user.type === 0 ? '(作者)' : '' }}:
					{{ e.content }}
				</div>
				<div class="item-comment">
					<div class="message">
